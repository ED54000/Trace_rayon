        ...
        for (int x = 0; x < largeur; x += cube) {
            for (int y = 0; y < hauteur; y += cube) {
                ...
                while (calcule) {
                    ...
                    try {
                        // Appel des threads
                        threadCalcul = new ThreadCalcul(x, y, hautLocale, largLocale, disp, services, rep);
                        threadCalcul.start();
                        calcule = false;
                    } catch (Exception e) {
                        services.remove(rep);
                        System.err.println("Service supprimé");
                    }

                    if (rep >= services.size()) {
                        rep = 0;
                    }
                }
